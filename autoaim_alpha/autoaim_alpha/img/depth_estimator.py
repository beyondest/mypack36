...
class Depth_Estimator:
    ...
    def get_result(self,input):
        ...
        Method:
            pnp: Perspective-n-Point algorithm.
                input: (big_rec, obj_class:str = 'small''big'): the coordinates of the object in the image.
        ...
        if self.method == "pnp":
            ...
            return self._PNP(big_rec,obj_class)
    ...
    def _PNP(self,big_rec,obj_class:str = 'small''big'):
        if big_rec is None:
        ...
        if obj_class == 'small':
            obj_points = self.pnp_params.small_obj_points
        elif obj_class == 'big':
            obj_points = self.pnp_params.big_obj_points
        else:
            lr1.critical(f'Object class Error {obj_class}, only support small and big')
            raise ValueError(f'Object class Error {obj_class}, only support small and big')
        
        
        obj_points = np.reshape(np.array(obj_points, dtype=np.double), (4, 3))
        ...
        if not success:
            lr1.warning(f'PnP Failed, big_rec: {big_rec}, obj_class: {obj_class}')
            return None
